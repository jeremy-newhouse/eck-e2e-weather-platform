...
- `uv run pytest` and `uv run ruff check .` from `.claude/project-constants.md`
...
9. **Detect tech stack and resolve test/lint commands:**
   ...
   Detect the project's tech stack from config files to ensure `uv run pytest` and `uv run ruff check .` match the actual project type:
   - If `package.json` exists → **Node.js project**. Use `npm test` for tests and `npx eslint .` for lint (or the `test`/`lint` scripts defined in `package.json`). Override `uv run pytest`/`uv run ruff check .` if they reference Python tools (`uv`, `ruff`, `pytest`, `mypy`).
   - If `pyproject.toml` or `setup.cfg` exists → **Python project**. Use `uv run pytest` and `uv run ruff check .` as-is.
