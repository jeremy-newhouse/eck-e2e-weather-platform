...
- `uv run pytest`, `uv run ruff check .`, `<FORMAT_COMMAND>` from project configuration

### Activities

#### Verification Steps (per affected repo)

1. **MUST** run tests: Execute `uv run pytest` in the repo
...
2. **Lint test code**: Run `uv run ruff check .` on the full codebase
...
4. Run uv run pytest after development to verify all tests pass.
...
- Scope boundary: "DO NOT modify files outside the scope of this issue"
- Use Write tool (not cat/echo/heredoc) for file creation
- Run `uv run ruff check .` on the full codebase
...
3. Each agent prompt MUST include:
   ...
   - Instruction to run `uv run ruff check .` on the full codebase
...
1. **MUST** run tests in all affected repos: `uv run pytest`
...
3. **MUST** run quality gates: `uv run ruff check .` and `<FORMAT_COMMAND>`
