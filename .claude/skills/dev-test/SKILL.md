...
- `.claude/project-constants.md` — `uv run pytest` fallback
...
3. Identify the test framework from project configuration:
   ...
   - Check `.claude/project-constants.md` for `uv run pytest` if framework is ambiguous
...
- Module-under-test analysis from Stage 2
- `uv run pytest` from `.claude/project-constants.md`
...
3. **MUST** execute `uv run pytest` from project-constants.md to verify all generated tests fail when run. Do NOT skip test execution.
